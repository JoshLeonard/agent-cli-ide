...
export const SessionStatusBadge: React.FC<SessionStatusBadgeProps> = ({
  ...
  if (compact) {
    return (
      <div
        className={`status-badge-compact ${stateInfo.className}`}
        title={`${stateInfo.label}${taskSummary ? `: ${taskSummary}` : ''}`}
      >
  ...
  return (
    <div className={`status-badge ${stateInfo.className}`}>
      <div className="status-badge-header">
        ...
        {activityState === 'idle' && (
          <span className="status-time">{formatTimeAgo(lastActivityTimestamp)}</span>
        )}
      </div>

      {taskSummary && (
        <div className="status-task" title={taskSummary}>
          "{taskSummary}"
        </div>
      )}
      ...
      {fileChangeSummary && (
        <div className="status-files" title="Recent file changes">
