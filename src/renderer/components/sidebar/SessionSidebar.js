...
export const SessionSidebar = ({ onSelectSession, onTerminateSession, }) => {
    ...
    const [worktreesError, setWorktreesError] = useState(null);
    ...
    const getWorktreeSession = (worktreePath) => {
        return sessionList.find(session => session.worktreePath === worktreePath && session.status === 'running');
    };
    ...
    return (<div className="session-sidebar">
      ...
      <div className="accordion-section">
        <button className="accordion-header" onClick={() => toggleSection('sessions')} aria-expanded={expandedSections.sessions}>
        ...
        <div className={`accordion-content ${expandedSections.sessions ? 'expanded' : ''}`}>
          <div className="session-list">
            {sessionList.length === 0 ? (<div className="no-items">
                <p>No active sessions</p>
              </div>) : (sessionList.map((session) => {
            const status = agentStatuses.get(session.id);
            return (<div key={session.id} className={`session-item ${activeSessionId === session.id ? 'active' : ''}`} onClick={() => onSelectSession(session.id)}>
                    <div className="session-item-header">
                      {status ? (<SessionStatusBadge activityState={status.activityState} taskSummary={null} recentFileChanges={[]} errorMessage={null} lastActivityTimestamp={status.lastActivityTimestamp} compact/>) : (<span className="session-status" style={{ backgroundColor: getStatusColor(session.status) }} title={session.status}/>)}
                      {session.agentIcon && (<span className="session-agent-icon" title={session.agentName}>
                          {session.agentIcon}
                        </span>)}
                    ...
                    {status && session.status === 'running' && (<SessionStatusBadge activityState={status.activityState} taskSummary={status.taskSummary} recentFileChanges={status.recentFileChanges} errorMessage={status.errorMessage} lastActivityTimestamp={status.lastActivityTimestamp}/>)}

                    <div className="session-item-details">
                      <div className="session-path" title={session.cwd}>
                        {formatPath(session.cwd)}
                      </div>
                      {session.branch && (<div className="session-branch">
                    ...
                    <div className="session-item-actions">
                      {session.status === 'running' && (<button className="session-action-btn terminate" onClick={(e) => {
                        e.stopPropagation();
      ...
      {/* Activity Section */}
      <div className="accordion-section">
        <button className="accordion-header" onClick={() => toggleSection('activity')} aria-expanded={expandedSections.activity}>
          <span className="accordion-toggle">
      ...
      {/* Worktrees Section */}
      <div className="accordion-section">
        <button className="accordion-header" onClick={() => toggleSection('worktrees')} aria-expanded={expandedSections.worktrees}>
          <span className="accordion-toggle">
        ...
        <div className={`accordion-content ${expandedSections.worktrees ? 'expanded' : ''}`}>
          <div className="worktree-list">
            {worktreesLoading ? (<div className="no-items">
                <p>Loading...</p>
              </div>) : worktreesError ? (<div className="no-items">
                <p>{worktreesError}</p>
              </div>) : additionalWorktrees.length === 0 ? (<div className="no-items">
                <p>No worktrees</p>
              </div>) : (additionalWorktrees.map((worktree) => {
            const activeSession = getWorktreeSession(worktree.path);
            return (<div key={worktree.path} className="worktree-item" draggable onDragStart={(e) => {
                    ...
                    <div className="worktree-item-header">
                      <span className="worktree-status" style={{
                    backgroundColor: activeSession
                      ...
                      <button className="worktree-delete-btn" onClick={() => handleDeleteWorktree(worktree)} title="Delete worktree">
                        &#128465;
                      </button>
                    </div>
          ...
          {isGitRepo && additionalWorktrees.length > 0 && (<div className="worktree-actions">
              <button className="clean-orphaned-btn" onClick={handleCleanOrphaned} title="Remove worktrees without active sessions">
                &#129529; Clean Orphaned
              </button>
            </div>)}
        </div>
