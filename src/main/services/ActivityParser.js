...
const PATTERNS = {
    // File operations
    fileCreated: [
        { pattern: /(?:Created|Wrote|Writing to)\s+(?:file:?\s+)?['"]?([^\s'"]+\.[a-zA-Z0-9]+)/gi, type: 'file_created' },
        { pattern: /(?:New file:?|Creating)\s+['"]?([^\s'"]+\.[a-zA-Z0-9]+)/gi, type: 'file_created' },
    ],
    fileModified: [
        { pattern: /(?:Edited|Modified|Updated|Updating|Edit)\s+['"]?([^\s'"]+\.[a-zA-Z0-9]+)/gi, type: 'file_modified' },
    ],
    fileDeleted: [
        { pattern: /(?:Deleted|Removed|Removing)\s+(?:file:?\s+)?['"]?([^\s'"]+\.[a-zA-Z0-9]+)/gi, type: 'file_deleted' },
    ],
    // Errors
    errors: [
        { pattern: /error TS(\d+):\s*(.+)/gi, extract: (match) => ({ code: `TS${match[1]}`, message: match[2] }) },
        { pattern: /^(?:Error|TypeError|ReferenceError|SyntaxError):\s*(.+)/gim, extract: (match) => ({ code: undefined, message: match[1] }) },
        { pattern: /^ERROR:\s*(.+)/gim, extract: (match) => ({ code: undefined, message: match[1] }) },
        { pattern: /npm ERR!\s*(.+)/gi, extract: (match) => ({ code: undefined, message: `npm: ${match[1]}` }) },
        { pattern: /fatal:\s*(.+)/gi, extract: (match) => ({ code: undefined, message: `git: ${match[1]}` }) },
    ],
    // Warnings
    warnings: [
        { pattern: /(?:warning|warn):\s*(.+)/gi, extract: (match) => ({ message: match[1] }) },
        { pattern: /WARN\s+(.+)/gi, extract: (match) => ({ message: match[1] }) },
    ],
    // Task completion
    taskCompleted: [
...
export class ActivityParser {
    lastParsedPosition = new Map();
    recentEvents = new Set(); // Deduplication
    ...
    createEvent(type, severity, title, context, filePath, details) {
        return {
            id: uuidv4(),
