...
export function registerIpcHandlers(mainWindow) {
    ...
    ipcMain.handle('session:terminate', async (_event, { sessionId }) => {
        return sessionRegistry.terminateSession(sessionId);
    });
    ipcMain.handle('session:write', (_event, { sessionId, data }) => {
    ...
    ipcMain.handle('session:resize', (_event, { sessionId, cols, rows }) => {
    ...
    ipcMain.handle('agent:get', (_event, { agentId }) => {
        return agentService.getAgent(agentId);
    });
    // Project handlers
    ipcMain.handle('project:open', async (_event, { path }) => {
    ...
    // Worktree handlers
    ipcMain.handle('worktree:list', async (_event, { repoPath }) => {
        return gitWorktreeManager.listWorktrees(repoPath);
    });
    ipcMain.handle('worktree:remove', async (_event, { worktreePath }) => {
    ...
    ipcMain.handle('worktree:isGitRepo', async (_event, { path }) => {
        return gitWorktreeManager.isGitRepo(path);
    });
    // Agent status handlers
    ipcMain.handle('agentStatus:get', (_event, { sessionId }) => {
    ...
    ipcMain.handle('activity:getEvents', (_event, filter) => {
        return activityFeedService.getEvents(filter);
    });
    ipcMain.handle('activity:clearEvents', (_event, { sessionId }) => {
        activityFeedService.clearEvents(sessionId);
        return { success: true };
    });
    // Messaging handlers
    ipcMain.handle('messaging:send', (_event, { targetSessionIds, content, options, }) => {
    ...
    ipcMain.handle('messaging:broadcast', (_event, { content, options, excludeSessionId, }) => {
        return messagingService.broadcast('', content, options, excludeSessionId);
    });
    ipcMain.handle('messaging:setClipboard', (_event, { content, sourceSessionId, }) => {
